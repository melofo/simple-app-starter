# Process
Since this task only requires to display the data, so most jobs have been done in the front-end(monolithstarter-static).
1. use datasetProcess.java to process the raw data
2. after the process, we will have two files: duplicate.csv and non-duplicate.csv
3. convert duplicate.csv and non-duplicate.csv to duplicate.json and non-duplicate.json
4. write .jsx
5. convert .jsx to .js
6. run
## dataset-process
This is the java app to process the raw data.
This app distinguishes the same issues and issues sound the same, and output the result in two files:
    non-duplicate.csv
    duplicate.csv
## front-end
Two more pages are created, one is for non-duplicate and the other one is for the duplicate.
These two pages were wrote in React .jsx and converted to React .js.
## result
The result can be seen in screenshots.
